...
class Local_Spectrum_SVGP(GPModel, ExternalDataTrainingLossMixin):
    ...
    Unlike the Tobar paper, we assume the time domain is windowed using a Dirichlet filter (as opposed to gaussian),
    which matches exactly the filter implied by the inducing variable. This keeps construction simple.
    """
    ...
    def elbo(self, data: RegressionData) -> tf.Tensor:
        ...
        var_exp = (self.likelihood.variational_expectations(tf.math.real(f_mean),
                                                            tf.math.real(f_var), tf.math.real(Y)) +
                   self.likelihood.variational_expectations(tf.math.imag(f_mean),
                                                            tf.math.imag(f_var), tf.math.imag(Y)))
